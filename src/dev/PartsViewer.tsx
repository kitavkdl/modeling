...
const URL = '/models/ninja400/parts.gltf'
...
function Model({ only, min, labels, dim }: { only: Set<number> | null; min: number; labels: boolean; dim: boolean }) {
  const gltf = useGLTF(URL)
  const items = useMemo(() => {
    const out: { i: number; mesh: THREE.Mesh; tris: number; center: THREE.Vector3 }[] = []
    gltf.scene.traverse((o) => {
      if (!(o as THREE.Mesh).isMesh) return
      const mesh = o as THREE.Mesh
      const i = Number(mesh.name.slice(1))
      const geo = mesh.geometry
      geo.computeBoundingBox()
      const center = new THREE.Vector3()
      geo.boundingBox!.getCenter(center)
      out.push({ i, mesh, tris: (geo.index?.count ?? 0) / 3, center })
    })
    return out.sort((a, b) => a.i - b.i)
  }, [gltf])
  return (
    <group scale={100}>
      {items.map(({ i, mesh, tris, center }) => {
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#202226' }}>
      <Canvas camera={{ position: pos, fov: 40, near: 1, far: 5000 }}>
        ...
        <Suspense fallback={null}>
          <Model only={only} min={min} labels={labels} dim={dim} />
        </Suspense>
